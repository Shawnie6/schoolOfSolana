...
pub fn remove_reaction(ctx: Context<RemoveReactionContext>) -> Result<()> {
    ...
    match tweet_reaction.reaction{
        ReactionType::Like => tweet.likes = tweet.likes.checked_sub(1).ok_or(TwitterError::MinLikesReached)?,
        ReactionType::Dislike => tweet.dislikes = tweet.dislikes.checked_sub(1).ok_or(TwitterError::MinDislikesReached)?
    };
    Ok(())
}
#[derive(Accounts)]
