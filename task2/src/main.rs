...
fn generate_key() {
    let base58_private_key = "3JhJvRB6LPssyBF8Pog98bYqKzfCJkJ1gmjduRoWDF6E4ZmtYtH3sPs8bt3fADBtsT6wSp8wzU3p8nBA9YtpBUfp";
    let private_key_bytes = bs58::decode(base58_private_key).into_vec().unwrap();
    let keypair_json = serde_json::to_string(&private_key_bytes).unwrap();
    fs::write("../../program-Shawnie6/anchor_project/campaigning/keypair.json", keypair_json).unwrap();
